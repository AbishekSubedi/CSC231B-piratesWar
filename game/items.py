...
class Item(superclasses.ActionResolver, context.Context):
    ...
    def getAttacks(self, owner):
        ...
        if (
            self.damage[1] > 0
            and not self.verb is None
            and not self.verb2 is None
            and self.skill in owner.skills.keys()
            and self.ready()
        ):
            attacks.append(
                superclasses.CombatAction(
                    self.verb + " with " + self.name,
                    superclasses.Attack(
                        self.name,
                        self.verb2,
                        owner.skills[self.skill],
                        self.damage,
                        self.firearm,
                    ),
                    self,
...
class Cutlass(Item):
    def __init__(self):
        super().__init__(
            "cutlass", 5
        )  # Note: price is in shillings (a silver coin, 20 per pound)
        self.damage = (10, 60)
        self.skill = "swords"
        self.verb = "slash"
        self.verb2 = "slashes"
